...
def read_and_preproces(filename, canny_low= 100, canny_high = 80, blur_kernel=9, d_e_kernel=3):#100,50
    # Đọc file ảnh
    image = cv2.imread(filename)
    # resize khung hình 640
...
image, edged = read_and_preproces('E:\\HocTap\\xla\\doKichThuoc2.jpg')
cv2.waitKey()
...
def find_object_in_pix(orig, edge, area_threshold=100):
    ...
    for c in cnts:
        ...
        if P is None:
            # Cập nhật số P
            P = ((ref_Width / dc_W)+(ref_High / dc_H))/2
